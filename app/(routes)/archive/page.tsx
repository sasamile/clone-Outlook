...
import { FolderArchivedMail, FolderDraftsMail, FolderInboxMail, FolderStarredMail } from "@/actions/folder-mail/send/inde";
...
function page() {
  ...
  const { data: sentEmails, isLoading } = useSWR(
    pathname === "/archive" ? "archive-emails" : null, // Solo cargar si estamos en /sent
    async () => {
      try {
        const emails = await FolderArchivedMail();
  ...
  return (
    <div className="bg-sidebar rounded-md h-full">
      <div className="py-4 flex justify-between items-center px-4">
        <h2 className="font-semibold ml-12">Archivados</h2>
